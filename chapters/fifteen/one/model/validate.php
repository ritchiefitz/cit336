...
class Validate {
    ...
    public function birthdate($name, $value, $required = true) {
        ...
        if ($match === false) {
            ...
            $field->setErrorMessage('Must match mm/dd/yyyy');
            return;
        }
        ...
        $dateString = $dateParts[0] . '/' . $dateParts[1] . '/' . $dateParts[2] . ' last day of 23:59:59';
        $birthdate = new DateTime($dateString);
        $now = new DateTime();
        if ( $birthdate > $now ) {
            $field->setErrorMessage('Birthdate cannot be in the future.');
            return;
        }
